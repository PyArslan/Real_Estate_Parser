...
class Save:
    ...
    def to_xlsx(estate_list, count=0):
        ...
        data = {
            ...
            'Цель': [i['Категория'].split(" / ")[1][:i['Категория'].split(" / ")[1].find(" ")] if 'Категория' in i else 'Не указано' for i in estate_list],
            ...
            'На каком этаже': [i['Этаж'] if 'Этаж' in i else 'Не указано' for i in estate_list],
            ...
            'Цена предложения ТМТ': [int(i['Цена']) if 'Цена' in i else 'Не указано' for i in estate_list],
            ...
            'Состояние ремонта': [i['Ремонт'] if 'Ремонт' in i else 'Не указано' for i in estate_list],
            'Web ссылка': [i['Web ссылка'] for i in estate_list],
            'Конт.номер': [i['Номер телефона'] if 'Номер телефона' in i else 'Не указано' for i in estate_list],
